	...
	, m_commandBuffer(VK_NULL_HANDLE)
{
	...
	if (vkAllocateCommandBuffers(device, &allocInfo, &m_commandBuffer) != VK_SUCCESS)
		throwWithLog<VulkanException>(
			std::source_location::current(), VulkanExceptionCause::ALLOCATE_COMMAND_BUFFERS);
}
...
std::vector<std::unique_ptr<VulkanCommandBuffer>> VulkanCommandBuffer::create(
	VkDevice device, VkCommandPool commandPool, std::uint32_t count, VkCommandBufferLevel level)
{
