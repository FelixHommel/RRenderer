...
VkResult VulkanSwapchain::submitCommandBuffer(
	const VkCommandBuffer* commandBuffer, const std::uint32_t* imageIndex)
{
	...
	if (vkQueueSubmit(device.getGraphicsQueueHandle(), 1, &submitInfo, m_inFlightFences[m_currentFrame]) !=
		VK_SUCCESS)
		throwWithLog<VulkanException>(
			std::source_location::current(), VulkanExceptionCause::QUEUE_SUBMIT_GRAPHICS);
	...
	const auto result{ vkQueuePresentKHR(device.getPresentQueueHandle(), &presentInfo) };
...
void VulkanSwapchain::createSwapchain(std::shared_ptr<VulkanSwapchain> previous)
{
	...
	VkSwapchainCreateInfoKHR createInfo{ .sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR,
										 ...
										 .clipped = VK_TRUE,
										 .oldSwapchain =
											 previous == nullptr ? VK_NULL_HANDLE : previous->getHandle() };

	const QueueFamilyIndices indices{ device.findPhysicalQueueFamilies() };
	if (!indices.areSameQueue())
	{
		const auto queueFamilyIndices{ indices.toAray() };
	...
	if (vkCreateSwapchainKHR(device.getHandle(), &createInfo, nullptr, &m_swapchain) != VK_SUCCESS)
		throwWithLog<VulkanException>(
			std::source_location::current(), VulkanExceptionCause::CREATE_SWAPCHAIN_KHR);
...
void VulkanSwapchain::createImageViews()
{
	...
	for (std::size_t i{ 0 }; i < m_swapchainImageViews.size(); ++i)
	{
		...
		if (vkCreateImageView(device.getHandle(), &createInfo, nullptr, &m_swapchainImageViews[i]) !=
			VK_SUCCESS)
			throwWithLog<VulkanException>(
				std::source_location::current(), VulkanExceptionCause::CREATE_IMAGE_VIEW, i);
	}
}
...
{
	const VkAttachmentDescription depthAttachment{ .format = findDepthFormat(),
	...
	const VkAttachmentDescription colorAttachment{ .format = m_swapchainImageFormat,
												   .samples = VK_SAMPLE_COUNT_1_BIT,
												   .loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR,
	...
	const VkRenderPassCreateInfo createInfo{ .sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO,
											 .attachmentCount =
	...
	if (vkCreateRenderPass(device.getHandle(), &createInfo, nullptr, &m_renderPass) != VK_SUCCESS)
		throwWithLog<VulkanException>(
			std::source_location::current(), VulkanExceptionCause::CREATE_RENDER_PASS);
}

/// \brief Set up the depth images, image views and their memory on the device
///
...
void VulkanSwapchain::createDepthResources()
{
	...
	for (std::size_t i{ 0 }; i < imageCount(); ++i)
	{
		...
		if (vkCreateImageView(device.getHandle(), &imageViewCreateInfo, nullptr, &m_depthImageViews[i]) !=
			VK_SUCCESS)
			throwWithLog<VulkanException>(
				std::source_location::current(), VulkanExceptionCause::CREATE_IMAGE_VIEW, i);
	}
...
void VulkanSwapchain::createFramebuffers()
{
	...
	for (std::size_t i{ 0 }; i < imageCount(); ++i)
	{
		const std::array<VkImageView, 2> attachments{ m_swapchainImageViews[i], m_depthImageViews[i] };

		const VkExtent2D swapchainExtent{ m_swapchainImageExtent };
		const VkFramebufferCreateInfo createInfo{ .sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO,
		...
		if (vkCreateFramebuffer(device.getHandle(), &createInfo, nullptr, &m_swapchainFramebuffers[i]) !=
			VK_SUCCESS)
			throwWithLog<VulkanException>(
				std::source_location::current(), VulkanExceptionCause::CREATE_FRAMEBUFFER, i);
	}
}
...
void VulkanSwapchain::createSyncObjects()
{
	...
	for (std::size_t i{ 0 }; i < m_renderFinishedSemaphores.size(); ++i)
	{
		if (vkCreateSemaphore(
				device.getHandle(), &semaphoreCreateInfo, nullptr, &m_renderFinishedSemaphores[i]) !=
			VK_SUCCESS)
			throwWithLog<VulkanException>(
				std::source_location::current(), VulkanExceptionCause::CREATE_SEMAPHORE, i);
	}

	for (std::size_t i{ 0 }; i < MAX_FRAMES_IN_FLIGHT; ++i)
	{
		if (vkCreateSemaphore(
				device.getHandle(), &semaphoreCreateInfo, nullptr, &m_imageAvailableSemaphores[i]) !=
				VK_SUCCESS ||
			vkCreateFence(device.getHandle(), &fenceCreateInfo, nullptr, &m_inFlightFences[i]) != VK_SUCCESS)
			throwWithLog<VulkanException>(
				std::source_location::current(), VulkanExceptionCause::CREATE_IN_FLIGHT_SYNC_OBJECT, i);
	}
}
...
VkExtent2D VulkanSwapchain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) const
{
	...
	actualExtent.width = std::max(
		capabilities.minImageExtent.width, std::min(capabilities.minImageExtent.width, actualExtent.width));
