	...
	: device(device)
{
	...
	if (vkCreateCommandPool(device.getHandle(), &createInfo, nullptr, &m_commandPool) != VK_SUCCESS)
		throwWithLog<VulkanException>(
			std::source_location::current(), VulkanExceptionCause::CREATE_COMMAND_POOL);
...
/// \sa VulkanCommandBuffer
std::vector<std::unique_ptr<VulkanCommandBuffer>> VulkanCommandPool::allocateCommandBuffer(
