...
VulkanInstance::VulkanInstance()
{
	if (useValidationLayers && !checkValidationLayerSupport())
		throwWithLog<VulkanException>(
			std::source_location::current(), VulkanExceptionCause::VALIDATION_LAYERS_UNAVAILABLE);
...
void VulkanInstance::hasGLFWRequiredInstanceExtensions()
{
	...
	for (const auto& required : requiredExtensions)
	{
		...
		if (!available.contains(required))
			throwWithLog<VulkanException>(
				std::source_location::current(), VulkanExceptionCause::GLFW_EXTENSIONS_MISSING);
	}
}
